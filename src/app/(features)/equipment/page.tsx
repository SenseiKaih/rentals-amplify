...
export default function Component() {
  ...
  const seasonItems = seasonss.map((season, index) => (
    <div key={index} className="bg-gray-100 p-4 rounded-lg shadow-md">
      <div className="h-32 bg-gray-300 mb-4"></div>
      <h3 className="text-center text-lg font-semibold">{season}</h3>
    </div>
  ))
    ...
    <div className="flex flex-col h-screen bg-gray-100">
      ...
      <header className="bg-white p-4 flex items-center justify-between shadow-sm">
        <div className="flex items-center space-x-2">
          <span className="font-bold text-black text-lg">FarmtexHub</span>
      ...
      >
        <div className="p-4">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-black text-lg font-semibold">Your Cart</h2>
          ...
          <div className="space-y-4 text-black">
            ...
            <Button className="w-full text-black" text-white bg-black>Checkout</Button>
            </Link>
          </div>
        </div>
      </div>
      ...
      <main className="flex-1 overflow-y-auto">
        ...
        <div className="p-4 bg-white sticky top-0 z-10 shadow-sm">
          <div className="relative flex items-center">
            ...
            <Input 
              className="pl-10 pr-20 text-black  py-2 w-full" 
        ...
        <div className="p-4 bg-white">
          <h2 className="text-lg text-black font-semibold mb-3">Rent By Category</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {equipmentCategories.map((category, index) => (
              <div key={index} className="relative group">
                ...
                <div className="relative left-0  mt-2 w-56 max-h-0 overflow-hidden opacity-0 invisible group-hover:opacity-100 group-hover:max-h-screen group-hover:visible transition-all duration-300 ease-in-out z-10">
                  <Card>
                    <CardContent className="p-2">
                      {category.subcategories.map((subcategory, subIndex) => (
                        <div key={subIndex} className="relative group/sub">
                          <Button className="w-full text-black  justify-start mb-1" variant="ghost">
                            {subcategory.name}
                            <ChevronRight className="ml-auto h-4 w-4" />
                          </Button>
                          <div className="relative  ml-2 w-48 opacity-0 max-h-0 overflow-hidden opacity-0 invisible   group-hover/sub:opacity-100 group-hover/sub:max-h-screen group-hover/sub:visible transition-all duration-300 ease-in-out">
                            <Card>
                              <CardContent className="p-2">
                                {subcategory.items.map((item, itemIndex) => (
                                  <Button key={itemIndex} className="w-full text-black  justify-start mb-1" variant="ghost">
                                    {item}
                                  </Button>
                                ))}
                              </CardContent>
        ...
        {/*Rent By Season */}
        <div className="p-4 bg-white">
          <h2 className="text-lg text-black font-semibold mb-3">Rent By Season</h2>
            <Carousel items={seasonItems} />
        </div>    
        ...
        <div className="p-4 bg-white mt-2">
          <div className="flex justify-between items-center mb-3">
            <h2 className="text-lg text-black  font-semibold">Featured Equipment</h2>
            <Button variant="ghost" size="sm" className="text-primary text-black">
          ...
          <div className="grid grid-cols-2 gap-4 text-black">
            {featuredEquipment.map((item, index) => (
              ...
              <Card key={index} className="overflow-hidden text-black">
                <CardHeader className="p-0 text-black">
                  <img
                    alt={item.name}
                ...
                <Link href="http://localhost:3000/listings/listings-detail">
                  ...
                  <CardDescription className="text-sm text-black">{item.description}</CardDescription>
                </CardContent>
              </Card>
            ))}
          </div>
        ...
        <div className="p-4">
          <div className="flex justify-between items-center mb-3 text-black">
            <h2 className="text-lg text-black font-semibold">Available Equipment</h2>
            <Button variant="ghost" size="sm" className="text-primary text-black">
              View All <ChevronRight className="ml-1 h-4 w-4" />
            </Button>
          </div>
            ...
            {filteredEquipment.map((item, index) => (
              <Card key={index} className="overflow-hidden text-black">
                <div className="md:flex text-black">
                  <CardHeader className="p-0 md:w-1/3 text-black">
                    <img
                      alt={item.name}
